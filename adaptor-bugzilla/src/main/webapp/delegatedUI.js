...
function select() {
  ...
  if( list.length>0 && list.selectedIndex >= 0 ) { // something is selected
    ...
    for (var item = 0; item < list.options.length; item++) {
      ...
      if (option.selected) {
        // from https://stackoverflow.com/questions/4253367/how-to-escape-a-json-string-containing-newline-characters-using-javascript
        var label = option.text.replace(/\\n/g, "\\n")
          .replace(/\\'/g, "\\'")
          .replace(/\\"/g, '\\"')
          .replace(/\\&/g, "\\&")
          .replace(/\\r/g, "\\r")
          ...
          .replace(/\\b/g, "\\b")
          .replace(/\\f/g, "\\f");
        oslcResponse +=  '{"oslc:label": "' + label + '", "rdf:resource": "' + option.value + '"}, '
      }
    }    
