...
public class RootServicesService extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * LAB 6 request.setAttribute("baseUri",
		 * BugzillaManager.getBugzServiceBase());
		 * request.setAttribute("catalogUri",
		 * ServiceProviderCatalogSingleton.getUri().toString());
		 * request.setAttribute("oauthDomain",
		 * BugzillaManager.getServletBase()); final RequestDispatcher rd =
		 * request.getRequestDispatcher("/cm/rootservices_rdfxml.jsp");
		 * rd.forward(request, response); response.flushBuffer();
		 */
	}
}
